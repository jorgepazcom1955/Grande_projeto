...
[
  ...
  // Verificação de código duplicado para evitar produtos duplicados
  console.log("Adicionando produto com código:", codigo);
  
  // Forçar o código a ser tratado como string para evitar problemas
  const codigoStr = String(codigo);
  
  // Verificar se já existe um produto com este código (apenas para novos produtos)
  if (index === '') {
    const produtoExistente = produtos.findIndex(p => p.id === codigoStr);
...
function salvarProduto() {
    ...
    // Verificar se já existe produto com este código (exceto o atual)
    if (produtos.some((p, i) => p.id === codigo && i != index)) {
        alert("Já existe um produto com este código!");
        return;
    };
